...
class Cache:
    ...
    def access_page(self, url, contents):
        ...
        if self.cache_dict.get(url) is None:
            if len(self.cache_dict) < self.size:
                node_place = self.cache_list.append((url, contents))
                self.cache_dict[url] = node_place
            else:
                deleted_node = self.cache_list.delete_head()
                self.cache_dict.pop(deleted_node.data[0])
                node_place = self.cache_list.append((url, contents))
                self.cache_dict[url] = node_place
        else:
            self.cache_list.delete_mid(self.cache_dict[url])
            node_place = self.cache_list.append((url, contents))
            self.cache_dict[url] = node_place
    ...
    def get_pages(self):
        ...
        ###########################
        ret_urls = []
        for url_and_web in self.cache_list.get_all():
            ret_urls.append(url_and_web[0])
        return ret_urls
